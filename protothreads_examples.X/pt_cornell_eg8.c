...
#define PB_FREQ SYS_FREQ/PB_DIVISOR     // PBCLK. 32MHz 32000000
...
#define max_chars 128 // Input buffer size
...
#define PT_RATE_SCHEDULE(f,rate) \
   if ((rate==0) | \
   (rate==1 && ((loop_priority & 0b1)==0) ) | \
   (rate==2 && ((loop_priority & 0b11)==0) ) | \
   (rate==3 && ((loop_priority & 0b111)==0) ) | \
   (rate==4 && ((loop_priority & 0b1111)==0) )) \
      PT_SCHEDULE(f);
...
static PT_THREAD(protothread3(struct pt *pt))
{
    ...
    while (1) { 
        ...
        if (cmd[0]=='p' && cmd[1]=='\0') {
            sprintf(send_buffer, "LED1Timer=%d LED2Timer=%d t1r=%d t2r=%d t3r=%d t4r=%d looppri=%X dma_cnt=%d\n\r", \
                    wait_t1, wait_t2, t1_rate, t2_rate, t3_rate, t4_rate, loop_priority, dma_cnt);
            PT_SPAWN(pt, &pt_DMA_output, DMA_PutSerialBuffer(&pt_DMA_output));
        }
    }
    PT_END(pt);
...
int main(void)
{
    ...
    while (1) {
        ...
        loop_priority = ++loop_priority & 0b1111;
    }
}